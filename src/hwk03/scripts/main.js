...
function videoHotspot(hotspotDiv, args) {
    ...
    // Custom Logic...
    span.innerHTML = `<div class="tooltip-title" style="color: white; text-align: center; font-size: 1.25rem;">${args.title}</div><video poster="./assets/pier-39-thumb.png" class="hotspot-vid" width="320" height="240" src="./assets/${args.src}" controls></video>`;
...
function imgHotspot(hotspotDiv, args) {
    ...
    const span = addSpan(hotspotDiv);
    // Custom Logic...
    span.innerHTML = `<div class="tooltip-title" style="color: white; text-align: center; font-size: 1.25rem;">${args.title}</div><img onclick="window.open('./assets/${args.src}', '_blank');" class="hotspot-img" src="./assets/${args.src}"/>`;
}